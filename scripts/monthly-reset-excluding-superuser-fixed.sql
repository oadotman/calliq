-- =====================================================
-- MONTHLY RESET SYSTEM - EXCLUDES SUPERUSER (FIXED)
...
WITH next_month_simulation AS (
    SELECT
        ...
        CASE
            WHEN u.email = 'adeliyitomiwa@yahoo.com' THEN o.used_minutes  -- Fixed: use o.used_minutes
            ELSE 0
...
SELECT
    'SUPERUSER STATUS:' as info;

SELECT
    u.email,
    o.name as organization,
    o.max_minutes_monthly as "monthly_limit",
    o.used_minutes as "current_usage",
    o.max_minutes_monthly - o.used_minutes as "remaining",
    'Will NOT reset on month change' as "reset_policy"
FROM users u
JOIN user_organizations uo ON uo.user_id = u.id
JOIN organizations o ON o.id = uo.organization_id
WHERE u.email = 'adeliyitomiwa@yahoo.com';

-- Show what will happen to all organizations on next reset
SELECT
    'NEXT MONTH RESET PREVIEW:' as info;
...
SELECT
    ...
    o.used_minutes as "current_usage",
    CASE
        WHEN su.email IS NOT NULL THEN o.used_minutes
        ELSE 0
    END as "usage_after_reset",
    CASE
        WHEN su.email IS NOT NULL THEN 'PRESERVED (Superuser)'
        ELSE 'RESET TO 0'
    END as "action"
FROM organizations o
LEFT JOIN (
...
ORDER BY
    CASE WHEN su.email IS NOT NULL THEN 0 ELSE 1 END,
    o.name;